...
@app.route('/api/parse/<ipv6_str>/<int:prefix_len>/<int:network_index>')
def parse_ipv6(ipv6_str, prefix_len, network_index=0):
    try:
        ...
        base_network = ipaddress.IPv6Network(f'{addr}/{prefix_len}', strict=False)
        
        # Calculate network offset
        network_size = 2 ** (64 - prefix_len)  # Size of each network
        offset = network_size * network_index
        
        try:
            # Calculate the target network address
            new_network_int = int(base_network.network_address) + (offset << 64)
            network = ipaddress.IPv6Network(f'{ipaddress.IPv6Address(new_network_int)}/{prefix_len}', strict=False)
        except:
            # If offset is invalid, return to base network
